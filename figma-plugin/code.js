...
// =============================================
// 後處理工具：批次建立 Component（從 Frame 子圖層）
// =============================================

async function handleCreateComponents() {
  var selection = figma.currentPage.selection;
  if (selection.length === 0) {
    figma.ui.postMessage({ type: 'comp-error', text: '⚠️ 請先選取包含子圖層的 Frame 或 Group' });
    return;
  }
  var targetFrame = selection[0];
  if (!('children' in targetFrame)) {
    figma.ui.postMessage({ type: 'comp-error', text: '⚠️ 選取的節點沒有子圖層，請選取 Frame 或 Group' });
    return;
  }
  var children = targetFrame.children.slice(); // snapshot，避免操作中被修改
  if (children.length === 0) {
    figma.ui.postMessage({ type: 'comp-error', text: '⚠️ 選取的 Frame 內沒有子圖層' });
    return;
  }

  var GAP     = 20;  // 元件間距
  var COLS    = 5;   // 每排最多幾個
  var PADDING = 20;  // 距離 Frame 邊緣的留白
  var curX    = PADDING;
  var curY    = PADDING;
  var rowMaxH = 0;
  var colCount = 0;
  ...
  for (var i = 0; i < children.length; i++) {
    ...
    var w = child.width  || 100;
    var h = child.height || 100;
    ...
    // Clone 子圖層內容放入 Component（x/y 歸零對齊左上角）
    var clone = child.clone();
    clone.x = 0;
    clone.y = 0;
    component.appendChild(clone);

    // 放置在原 Frame 內部（座標相對於 Frame）
    component.x = curX;
    component.y = curY;
    targetFrame.appendChild(component);
  ...
  figma.ui.postMessage({
    type: 'comp-done',
    text: '✅ 完成！共建立 ' + created.length + ' 個 Component（放置於選取的 Frame 內）'
  });
}
